...
import { processAcademicPeriod } from "@/src/utils/periodic/processAcademicPeriod";
import {
  getAcademicSnapshots,
  saveAcademicSnapshot,
  deleteAcademicSnapshot,
} from "@/src/utils/periodic/academicStorage";
...
import { detectAcademicPeriod } from "@/src/utils/periodic/detectAcademicPeriod";
...
export function SettingsFeacture() {
  ...
  return (
    <section className="h-full w-full bg-slate-50 p-3 md:p-4 flex flex-col overflow-y-auto md:overflow-hidden">
      ...
      {/* Contenido */}
      <section className="flex w-full gap-3 md:gap-4 flex-1 min-h-0">
        {/* Upload */}
        <div className="w-full rounded-xl border border-border bg-white p-3 md:p-4 shadow-sm flex flex-col min-h-0">
        ...
        {/* Documentos */}
        <div className="hidden md:flex max-w-100 w-145 h-full bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex-col space-y-2">
          ...
          {snapshots.length === 0 ? (
            <div className="flex-1 rounded-xl flex flex-col items-center justify-center text-center text-gray-400 p-4">
              ...
              <p className="text-sm md:text-lg">No hay datos cargados.</p>
            </div>
          ) : (
